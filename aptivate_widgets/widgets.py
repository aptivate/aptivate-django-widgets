...
class YearMonthDateSelectorWidget(MultiWidget):
    def __init__(self, attrs=None):
        ...
        wattrs = {
            ...
            'class': 'year-month-date-selector'
        }
        if attrs:
            wattrs.update(attrs)
        months = [(1, 'Janury'),
                  ...
                  (11, 'November'),
                  (12, 'December')]
        years = [(year, year) for year in range(wattrs['min_year'],
                                                wattrs['max_year'])]
        months.insert(0, ('', '----'))
        years.insert(0, ('', '----'))
        widgets = (Select(attrs=wattrs, choices=months),
                   Select(attrs=wattrs, choices=years))
        super(YearMonthDateSelectorWidget, self).__init__(widgets, wattrs)
